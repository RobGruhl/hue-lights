#!/bin/bash
# Nordic Twilight - Cozy hygge-style relaxation scene
# Soft lavender → dusty rose → warm cream → pale amber
# Very slow drift, almost imperceptible - perfect for unwinding
...
# Nordic palette - muted, cozy tones
# Soft lavender → dusty rose → blush → warm cream → pale amber
# Brightness moderate for cozy feel
declare -a CX=(0.34 0.38 0.42 0.44 0.46 0.48 0.46 0.44 0.42 0.40 0.38 0.36)
declare -a CY=(0.28 0.30 0.34 0.36 0.38 0.40 0.41 0.39 0.36 0.33 0.30 0.28)
declare -a CB=(70   75   80   85   85   85   85   80   75   70   70   70)

PALETTE_LEN=${#CX[@]}

# Position offsets for very slow wave
POS_OFFSETS=(0 1 2 3 4)
...
set_gradient_light() {
    ...
    curl -sk -X PUT "https://$HUE_BRIDGE/clip/v2/resource/light/$id" \
      ...
      ]},\"dynamics\":{\"duration\":12000},\"dimming\":{\"brightness\":75}}" > /dev/null 2>&1
}
...
set_solid_light() {
    ...
    curl -sk -X PUT "https://$HUE_BRIDGE/clip/v2/resource/light/$id" \
      -H "hue-application-key: $HUE_USER" \
      -H "Content-Type: application/json" \
      -d "{\"color\":{\"xy\":{\"x\":${CX[$p]},\"y\":${CY[$p]}}},\"dimming\":{\"brightness\":${CB[$p]}},\"dynamics\":{\"duration\":12000}}" > /dev/null 2>&1
}

echo "NORDIC TWILIGHT: Cozy hygge vibes (very slow drift)"
echo "Press Ctrl+C to stop"

phase=0
while true; do
    # Signes with gentle wave
    ...
    for id in "${KITCHEN_WEST[@]}"; do
        set_solid_light "$id" $(( phase % PALETTE_LEN )) &
    done
    for id in "${KITCHEN_EAST[@]}"; do
        set_solid_light "$id" $(( (phase + 5) % PALETTE_LEN )) &
    done

    wait
    phase=$(( (phase + 1) % PALETTE_LEN ))
    sleep 15  # Very slow - 15 seconds per step
done
